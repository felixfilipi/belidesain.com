...
#tiket
select expotransactionheader.ticketqty*
...
Delimiter $$
create procedure felix(in tanggal_param date)
...
select tanggal_penjualan, max(total) as maksimal_penjualan, min(total) as minimal_penjualan
from(select cast(tanggal as date) as tanggal_penjualan,sum(penjualan) as total 
from temp1 group by tanggal_penjualan) a ;
end $$
Delimiter ;
...
if min_max='min' then
select tanggal1, min(a.total) as minimal_penjualan
from(select cast(tanggal as date) as tanggal1, sum(penjualan) as total 
from temp2 group by cast(tanggal as date)) a ,temp1
where
	temp2.tanggal = tanggal1;
elseif min_max='max' then 
select tanggal, max(total) as maksimal_penjualan
from(select cast(tanggal as date) as tanggal_penjualan, sum(penjualan) as total 
from temp1 group by cast(tanggal as date)) a ;
end if;
end $$
Delimiter ;
drop procedure filipi;
drop table temp2;
call filipi('min');

DELIMITER $$
CREATE PROCEDURE GetUserMaxMin(
	IN order_max_min VARCHAR(20)
)
BEGIN
	SELECT
		UserInfo.Name,
...
ORDER BY
	...
	delimiter ;
    call procedure1('desc');
    drop procedure procedure1;
    
    SELECT *FROM EXPOEVENTDETAILS;
